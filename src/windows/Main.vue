...
import type { CubismExpressionRequest, CubismHitAreaInfo } from '@/utils/cubism'
...
type Live2DModelClickPayload = {
  x: number
  y: number
  hitArea?: CubismHitAreaInfo | null
}

function createDefaultLive2DClickSettings(): Live2DClickSettings {
  return {
    enabled: false,
    headEnabled: true,
    bodyEnabled: true,
...
function normalizeLive2DClickSettings(value: unknown): Live2DClickSettings {
  const defaults = createDefaultLive2DClickSettings()
  ...
  return {
    ...
    headMotionIndex: Math.floor(
      finiteNumber(source.headMotionIndex, defaults.headMotionIndex, 0, 999)
    ),
    ...
    bodyMotionIndex: Math.floor(
      finiteNumber(source.bodyMotionIndex, defaults.bodyMotionIndex, 0, 999)
    ),
...
function loadLive2DClickSettings(): Live2DClickSettings {
  try {
    const raw = localStorage.getItem(LIVE2D_CLICK_INTERACTION_SETTINGS_KEY)
    return normalizeLive2DClickSettings(raw ? JSON.parse(raw) : null)
  } catch (error) {
    console.warn('[主窗口] 读取 Live2D 点击交互配置失败:', error)
    return createDefaultLive2DClickSettings()
  }
}

const clickInteractionSettings = ref<Live2DClickSettings>(loadLive2DClickSettings())

function refreshLive2DClickSettings() {
  clickInteractionSettings.value = loadLive2DClickSettings()
}
...
const lastClickActionAt = new Map<string, number>()
...
function scheduleExpressionReset(element: PerformElement, holdMs: number) {
  const timer = setTimeout(() => {
    ...
    if (element.resetPolicy === 'fadeOut') {
      live2dCanvasRef.value?.resetExpression(300)
    } else if (element.resetPolicy === 'default') {
      live2dCanvasRef.value?.setExpression({ id: 'default', fade: 300 })
    }
  }, holdMs)
...
function inferClickZone(payload: Live2DModelClickPayload, settings: Live2DClickSettings): Live2DClickZone {
  ...
  if (/body|torso|arm|hand|chest|waist|身体|体|手|臂|胸|腰/.test(hitAreaText)) {
    return 'body'
  }

  const bounds = live2dCanvasRef.value?.getModelOverlayBounds()
  if (bounds && bounds.height > 0) {
    return payload.y <= bounds.top + bounds.height * settings.headRatio ? 'head' : 'body'
  }

  return 'body'
}

function getFirstMotionCandidate(): { group: string; index: number } | null {
  const info = live2dCanvasRef.value?.getModelInfo()
  if (!info) {
    return null
  }

  for (const [group, motions] of Object.entries(info.motionGroups)) {
    if (Array.isArray(motions) && motions.length > 0) {
      return { group, index: motions[0]?.index ?? 0 }
    }
  }
...
function getFirstExpressionCandidate(): string {
  const expressions = live2dCanvasRef.value?.getModelInfo().expressions ?? []
  return expressions[0] ?? ''
}

function triggerClickExpression(expression: string, settings: Live2DClickSettings): boolean {
  if (!expression) {
    return false
  }
...
function handleModelClick(payload: Live2DModelClickPayload) {
  if (showMenu.value) {
  ...
  const settings = clickInteractionSettings.value
  if (!settings.enabled) {
    return
  }

  const zone = inferClickZone(payload, settings)
  const zoneEnabled = zone === 'head' ? settings.headEnabled : settings.bodyEnabled
  if (!zoneEnabled) {
    return
  }

  const actionKey = `${zone}:${payload.hitArea?.id ?? payload.hitArea?.name ?? 'fallback'}`
  const now = Date.now()
  const lastTriggeredAt = lastClickActionAt.get(actionKey) ?? 0
  if (settings.cooldownMs > 0 && now - lastTriggeredAt < settings.cooldownMs) {
    return
  }

  let motionGroup = zone === 'head' ? settings.headMotionGroup : settings.bodyMotionGroup
  let motionIndex = zone === 'head' ? settings.headMotionIndex : settings.bodyMotionIndex
  let expression = zone === 'head' ? settings.headExpression : settings.bodyExpression

  if (settings.fallbackToFirst && !motionGroup && !expression) {
    const firstMotion = getFirstMotionCandidate()
    if (firstMotion) {
      motionGroup = firstMotion.group
      motionIndex = firstMotion.index
    }
    expression = getFirstExpressionCandidate()
  }

  let triggered = false
  if (motionGroup) {
    stopIdleForPerformance()
    live2dCanvasRef.value?.playMotion(motionGroup, motionIndex, settings.priority, false)
    triggered = true
  }

  if (triggerClickExpression(expression, settings)) {
    triggered = true
  }
  ...
  lastClickActionAt.set(actionKey, now)
  if (settings.showToast) {
    showBaseEventStatus(`点击交互：${zone === 'head' ? '头部' : '身体'}`, 'info', 1200)
  }
}
...
function handleStorageChange(event: StorageEvent) {
  if (
    event.key &&
    event.key !== ADVANCED_SETTINGS_KEY &&
    event.key !== LIVE2D_CLICK_INTERACTION_SETTINGS_KEY
  ) {
    return
  }

  if (!event.key || event.key === LIVE2D_CLICK_INTERACTION_SETTINGS_KEY) {
    refreshLive2DClickSettings()
  }

  if (event.key === LIVE2D_CLICK_INTERACTION_SETTINGS_KEY) {
    return
  }
  ...
  mainWindowDisposers.push(
    window.electron.model.onPreviewExpression(payload => {
      ...
      live2dCanvasRef.value?.setExpression({
        ...
        fadeOut: payload.fadeOut ?? payload.fade ?? 200,
        holdMs,
        resetPolicy
      })
    })
