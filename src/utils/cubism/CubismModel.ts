...
export class CubismModel {
  ...
  private scheduleLegacyExpressionHold(
    ...
    const fadeOutMs = this.normalizeExpressionFadeMs(request.fadeOut ?? request.fade, 300)
    const now = Date.now()
  ...
  private beginExpressionFadeOut(fadeMs: number): void {
    const normalizedFadeMs = this.normalizeExpressionFadeMs(fadeMs, 300)
    const now = Date.now()

    if (this.activeExpressionRuntime) {
      this.activeExpressionRuntime.resetPolicy = 'neutral'
      this.activeExpressionRuntime.holdUntil = null
      this.activeExpressionRuntime.previous = null
      this.activeExpressionRuntime.previousLegacyExpressionName = null
      this.activeExpressionRuntime.fadeOutMs = normalizedFadeMs
      this.activeExpressionRuntime.fadeOutStartedAt = now
      return
    }

    if (this.activeLegacyExpressionName) {
      this.activeExpressionRuntime = {
        members: [],
        previous: null,
        previousLegacyExpressionName: null,
  ...
  focus(x: number, y: number): void {
    ...
    // 睡觉补丁：全屏鼠标模拟轻量动捕。
    // 原版只在鼠标靠近模型时跟随，离开后回正并继续叠加随机待机噪声，
    // 桌面常驻时会显得“缺动捕乱动”。这里让鼠标始终作为头部/眼球目标。
    this.gazePointerInRange = true

    const headX = (bounds.left + bounds.right) / 2
    const headY = bounds.top + bounds.height * 0.2
    const viewportWidth = Math.max(1, window.innerWidth || bounds.width || 1)
    const viewportHeight = Math.max(1, window.innerHeight || bounds.height || 1)

    // 用视口尺度归一化，而不是模型附近小范围；鼠标离远也只是轻轻看过去，不会突然归零。
    const normalizedX = Math.min(1, Math.max(-1, (x - headX) / (viewportWidth * 0.42)))
    const normalizedY = Math.min(1, Math.max(-1, -(y - headY) / (viewportHeight * 0.42)))

    // 压低幅度，避免大头/小模型看起来像被甩。
    this.dragManager.set(normalizedX * 0.55, normalizedY * 0.45)
  }
  ...
  resetExpression(fadeMs: number = 300): void {
    this.beginExpressionFadeOut(fadeMs)
  }
  ...
  getHitAreas(): CubismHitAreaInfo[] {
    if (!this.modelSetting) {
      return []
    }

    const hitAreas: CubismHitAreaInfo[] = []
    const seen = new Set<string>()
    const hitAreaCount = this.modelSetting.getHitAreasCount()
    for (let i = 0; i < hitAreaCount; i++) {
      ...
      seen.add(key)
      hitAreas.push({ id, name })
    }

    return hitAreas
  }

  getHitAreaAtPoint(x: number, y: number): CubismHitAreaInfo | null {
    if (!this.userModel || !this.modelSetting) return null
  ...
  isPointInModel(x: number, y: number): boolean {
    ...
    if (this.getHitAreaAtPoint(x, y)) return true
