...
function lpkSafeName(value: unknown, fallback = 'model'): string {
  const safe = String(value || fallback || 'model')
    .replace(/\.(lpk|model3|json|exp3|motion3)$/gi, '')
    .replace(/[<>:"/\\|?*\u0000-\u001f]/g, '_')
    .trim()
    .replace(/[. ]+$/g, '')
...
function lpkUniquePath(requestedPath: string, usedPaths: Set<string>): string {
...
function lpkEnsureInside(rootDir: string, relativePath: string): string {
  const resolvedRoot = path.resolve(rootDir)
  const resolvedPath = path.resolve(rootDir, relativePath)
  const rootWithSep = resolvedRoot.endsWith(path.sep) ? resolvedRoot : resolvedRoot + path.sep
  if (!resolvedPath.toLowerCase().startsWith(rootWithSep.toLowerCase())) {
    throw new Error('LPK 包含不安全路径')
  }
  return resolvedPath
}

function lpkExtractZip(lpkPath: string, targetDir: string): void {
  const buffer = fs.readFileSync(lpkPath)
  let eocdOffset = -1
  ...
  for (let index = 0; index < entryCount; index++) {
    ...
    const entryName = buffer
      .slice(centralOffset + 46, centralOffset + 46 + fileNameLength)
      .toString('utf8')
      .replace(/\\/g, '/')
    ...
    if (path.isAbsolute(normalizedEntryName) || normalizedEntryName.split(/[\\/]+/).includes('..')) {
      throw new Error('LPK 包含不安全路径')
    }
    ...
    if (compressionMethod === 0) {
      ...
      throw new Error(`LPK ZIP 压缩方式不支持: ${compressionMethod}`)
    }

    const outputPath = lpkEnsureInside(targetDir, normalizedEntryName)
    fs.mkdirSync(path.dirname(outputPath), { recursive: true })
    fs.writeFileSync(outputPath, fileData)
  }
}

function lpkHash(value: string): number {
  let hash = 0
...
function lpkDecrypt(buffer: Buffer, seed: number): Buffer {
  const output = Buffer.allocUnsafe(buffer.length)
  for (let offset = 0; offset < buffer.length; offset += 1024) {
    let random = seed
    const end = Math.min(offset + 1024, buffer.length)
    for (let index = offset; index < end; index++) {
      random = Math.floor((2531011 + 214013 * random) / 65536) & 65535
      output[index] = (random & 255) ^ buffer[index]
    }
  }
  return output
}

function lpkReadJson(input: Buffer | string, label = ''): any {
  try {
    const text = Buffer.isBuffer(input) ? input.toString('utf8') : fs.readFileSync(input, 'utf8')
    return JSON.parse(text)
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    throw new Error(`LPK JSON 解析失败 ${label}: ${message}`)
  }
}

function lpkFindManifest(extractDir: string): any {
  const entries = fs.readdirSync(extractDir, { withFileTypes: true })
  for (const entry of entries) {
    if (!entry.isFile() || entry.name.toLowerCase().endsWith('.bin3')) {
      continue
    }

    const filePath = path.join(extractDir, entry.name)
    try {
      const json = lpkReadJson(filePath, entry.name)
      if (json && json.type === 'STD2_0' && json.id) {
        return json
      }
    } catch {
      // Non-manifest entries are ignored.
    }
  }
...
async function prepareLpkImport(sourcePath: string, requestedModelName: string): Promise<LpkPrepareResult | null> {
  const lpkPath = String(sourcePath || '')
  if (!lpkPath.toLowerCase().endsWith('.lpk')) {
    return null
  }

  if (!fs.existsSync(lpkPath) || !fs.statSync(lpkPath).isFile()) {
    throw new Error('请选择有效的 LPK 文件')
  }

  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'l2dpro-lpk-'))
  const extractDir = path.join(tempDir, 'extract')
  const modelDir = path.join(tempDir, 'model')

  try {
    lpkExtractZip(lpkPath, extractDir)
    fs.mkdirSync(modelDir, { recursive: true })

    const manifest = lpkFindManifest(extractDir)
    const costume = manifest.list?.[0]?.costume?.[0]
    if (!costume?.path) {
      throw new Error('LPK 清单没有模型入口')
    }

    const manifestId = String(manifest.id)
    const readEncryptedResource = (resourcePath: string): Buffer => {
      const absolutePath = lpkEnsureInside(extractDir, resourcePath)
      if (!fs.existsSync(absolutePath)) {
        throw new Error(`LPK 缺少资源: ${resourcePath}`)
      }
      return lpkDecrypt(fs.readFileSync(absolutePath), lpkHash(manifestId + resourcePath))
    }

    const modelJson = lpkReadJson(readEncryptedResource(costume.path), costume.path)
    const fileReferences = modelJson.FileReferences || (modelJson.FileReferences = {})
    const usedPaths = new Set<string>()
    const convertedPaths = new Map<string, string>()

    const writeResource = (resourcePath: string, outputPath: string): string => {
      if (!resourcePath) {
        return ''
      }
      const cached = convertedPaths.get(resourcePath)
      if (cached) {
        return cached
      }

      const uniquePath = lpkUniquePath(outputPath.replace(/\\/g, '/'), usedPaths)
      const targetPath = path.join(modelDir, uniquePath)
      fs.mkdirSync(path.dirname(targetPath), { recursive: true })
      fs.writeFileSync(targetPath, readEncryptedResource(resourcePath))
      convertedPaths.set(resourcePath, uniquePath)
      return uniquePath
    }

    if (fileReferences.Moc) {
      fileReferences.Moc = writeResource(fileReferences.Moc, 'Moc.moc3')
    }
    if (Array.isArray(fileReferences.Textures)) {
      fileReferences.Textures = fileReferences.Textures.map((texture: string, index: number) =>
        writeResource(texture, `Textures/texture_${String(index).padStart(2, '0')}.png`)
      )
    }
    if (fileReferences.Physics) {
      fileReferences.Physics = writeResource(fileReferences.Physics, 'Physics.json')
    }
    if (fileReferences.PhysicsV2?.File) {
      const physicsPath = writeResource(fileReferences.PhysicsV2.File, 'Physics.json')
      fileReferences.PhysicsV2.File = physicsPath
      if (!fileReferences.Physics) {
        fileReferences.Physics = physicsPath
      }
    }
    if (fileReferences.Pose) {
      fileReferences.Pose = writeResource(fileReferences.Pose, 'Pose.json')
    }
    if (fileReferences.UserData) {
      fileReferences.UserData = writeResource(fileReferences.UserData, 'UserData.json')
    }
    if (Array.isArray(fileReferences.Expressions)) {
      fileReferences.Expressions = fileReferences.Expressions.map((entry: any, index: number) => {
        if (entry?.File) {
          entry.File = writeResource(
            entry.File,
            `Expressions/${lpkSafeName(entry.Name || `Expression_${index}`, `Expression_${index}`)}.exp3.json`
          )
        }
        return entry
      })
    ...
    if (fileReferences.Motions && typeof fileReferences.Motions === 'object') {
      for (const [group, motions] of Object.entries(fileReferences.Motions)) {
        ...
        motions.forEach((entry: any, index: number) => {
          if (entry?.File) {
            entry.File = writeResource(
              entry.File,
              `Motions/${lpkSafeName(`${group}_${entry.Name || index}`, `motion_${index}`)}.motion3.json`
            )
          }
        })
      }
    ...
    const modelName = lpkSafeName(
      String(requestedModelName || path.basename(lpkPath, path.extname(lpkPath))).replace(/\.lpk$/i, ''),
      lpkSafeName(manifest.name || manifest.list?.[0]?.character || costume.name || 'model')
    )
    const modelFileName = `${modelName}.model3.json`
    fs.writeFileSync(path.join(modelDir, modelFileName), JSON.stringify(modelJson, null, 2), 'utf8')
    return { sourceDir: modelDir, modelName, tempDir }
  } catch (error) {
    await removeDirectoryWithRetry(tempDir).catch(() => {})
    throw error
  }
}
...
ipcMain.handle('model:selectFolder', async () => {
  ...
  try {
    ...
    const sourceChoice = mainWindow
      ? await dialog.showMessageBox(mainWindow, sourceChoiceOptions)
    ...
    const isLpk = sourceChoice.response === 1
    const dialogOptions = isLpk
      ? {
          title: '选择 Live2DViewerEX LPK 文件',
          properties: ['openFile'] as Array<'openFile'>,
          filters: [
            { name: 'Live2DViewerEX Package', extensions: ['lpk'] },
    ...
    timer.done({ canceled: false, folderPath, sourceType: isLpk ? 'lpk' : 'folder' })
    return { success: true, folderPath, sourceType: isLpk ? 'lpk' : 'folder' }
  } catch (error: any) {
    console.error('[IPC] 选择模型来源失败:', error)
    timer.fail(error)
...
ipcMain.handle('model:import', async (_event, sourceDir: string, modelName: string) => {
  ...
  let lpkTempDir = ''
  try {
    const preparedLpk = await prepareLpkImport(sourceDir, modelName)
    if (preparedLpk) {
      sourceDir = preparedLpk.sourceDir
      modelName = preparedLpk.modelName
      lpkTempDir = preparedLpk.tempDir
    }
    ...
    const sourceType = lpkTempDir ? 'lpk' : 'folder'
    ...
    if (lpkTempDir) {
      await removeDirectoryWithRetry(lpkTempDir).catch(error => {
        console.warn('[IPC] 清理 LPK 临时目录失败:', error)
      })
    }
  }
})
